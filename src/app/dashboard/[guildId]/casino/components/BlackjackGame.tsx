...
const RESULT_CONFIG: Record<string, { label: string; color: string; glow: string; bg: string }> = {
    WIN:       { label: "WIN",       color: "text-emerald-400", glow: "shadow-[0_0_40px_rgba(16,185,129,0.6)]",  bg: "bg-[var(--bg-elevated)]merald-500/20 border-emerald-500/50"  },
...
function ChipToken({ amount, size = "md" }: { amount: number; size?: "sm" | "md" | "lg" }) {
    ...
    return (
        ...
        >
            <div className="absolute inset-[3px] rounded-full border border-[var(--border)]"/>
...
function StatusBadge({ status }: { status: string }) {
    const config: Record<string, { label: string; className: string }> = {
        IDLE:          { label: "Spectating", className: "text-[var(--text-tertiary)] bg-gray-500/10 border-gray-500/20" },
        BET_PLACED:    { label: "✓ Bet In",   className: "text-emerald-400 bg-[var(--bg-elevated)]merald-500/10 border-emerald-500/20" },
        ...
        STAND:         { label: "Stand",      className: "text-amber-400 bg-[var(--bg-secondary)]mber-500/10 border-amber-500/20" },
    ...
    const c = config[status] || { label: status, className: "text-[var(--text-secondary)] bg-gray-500/10 border-gray-500/20" };
    return (
    ...
    if (!gameState || !currentLiveState) {
        return (
            <div className="w-full h-full flex flex-col items-center justify-center p-8 bg-[#070710]">
                ...
                <p className="text-[var(--text-tertiary)] font-black uppercase tracking-[0.3em] text-xs">Loading Table...</p>
            </div>
        );
    }
    ...
    return (
        <div className="w-full h-full flex flex-col overflow-hidden relative" style={{ background: "radial-gradient(ellipse at 50% 0%, #0d1f15 0%, #070710 50%, #050508 100%)" }}>
            ...
            <div className="absolute inset-0 pointer-events-none overflow-hidden">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-[var(--bg-elevated)]merald-900/25 blur-[120px] rounded-full"/>
            ...
            {/* ── Top bar ── */}
            <div className="relative z-20 flex items-center justify-between px-4 sm:px-6 pt-3 pb-2 border-b border-[var(--border)] bg-[var(--bg-tertiary)] backdrop-blur-md flex-shrink-0">
                <button
                    onClick={onLeaveTable}
                    className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-[var(--text-tertiary)] hover:text-white transition-colors group"
                >
                ...
                <div className="flex items-center gap-2">
                    {status === 'WAITING' && (
                        <div className="flex items-center gap-2 bg-[var(--bg-secondary)]mber-500/10 border border-amber-500/30 rounded-full px-4 py-1.5">
                            <span className="w-1.5 h-1.5 rounded-full bg-[var(--bg-secondary)]mber-400 animate-pulse"/>
                    ...
                    {status === 'PLAYING' && (
                        <div className="flex items-center gap-2 bg-[var(--bg-elevated)]merald-500/10 border border-emerald-500/30 rounded-full px-4 py-1.5">
                ...
                {/* My chips */}
                <div className="flex items-center gap-2 bg-[var(--bg-hover)] border border-[var(--border)] rounded-full px-3 py-1.5">
                ...
                {notification && (
                    <motion.div
                        ...
                        className={`absolute bottom-32 left-1/2 -translate-x-1/2 z-50 px-6 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest border backdrop-blur-md ${
                            ...
                            notification.type === 'success' ? 'bg-[var(--bg-elevated)]merald-500/20 border-emerald-500/50 text-emerald-400' :
                            'bg-[var(--bg-hover)] border-white/20 text-white'
                        }`}
                    >
                        {notification.text}
                    </motion.div>
            ...
            <AnimatePresence>
                {showResultPopup && status === 'FINISHED' && myResult && (
                    ...
                    >
                        <div className={`relative border rounded-xl p-10 flex flex-col items-center backdrop-blur-xl shadow-2xl pointer-events-auto ${RESULT_CONFIG[myResult.type]?.bg || 'bg-[var(--bg-hover)] border-white/20'} ${RESULT_CONFIG[myResult.type]?.glow || ''}`}>
                            {/* Stars for blackjack */}
                            ...
                            {myResult.amount_won > 0 ? (
                                <span className="text-white text-2xl font-black flex items-center gap-2">
                                    <span className="text-emerald-400">+</span>{myResult.amount_won.toLocaleString()} <span className="text-[var(--text-secondary)] text-sm">chips</span>
                                </span>
                            ) : myResult.type === 'PUSH' ? (
            ...
            <div className="flex-1 min-h-0 flex flex-col items-center justify-around px-4 py-2 relative z-10 overflow-hidden">
                ...
                <div className="flex flex-col items-center w-full max-w-4xl">
                    ...
                    <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="flex items-center gap-2 bg-black/40 border border-[var(--border)] rounded-full px-4 py-1.5 backdrop-blur-md">
                            <div className="w-6 h-6 rounded-full bg-[var(--bg-elevated)]merald-800/50 border border-emerald-500/30 flex items-center justify-center text-sm">🕴️</div>
                            <span className="text-[10px] font-black uppercase tracking-widest text-[var(--text-secondary)]">The Dealer</span>
                            {/* Show dealer total (only visible cards) */}
                            {currentLiveState.dealer_cards?.length > 0 && status !== 'WAITING' && (
                                <span className="bg-[var(--bg-hover)] border border-[var(--border)] text-white px-2 py-0.5 rounded-md text-[10px] font-black tabular-nums">
                                ...
                                <div key={idx} className="flex flex-col items-center gap-2">
                                    ...
                                    {player ? (
                                        ...
                                        >
                                            ...
                                            <div className={`flex gap-1 justify-center items-end relative ${isBusted ? 'opacity-50' : ''}`} style={{ minHeight: '5.5rem' }}>
                                                <AnimatePresence>
                                                    {player.cards?.length > 0 ? player.cards.map((card: any, cIdx: number) => (
                                                        ...
                                                        status === 'WAITING' ? (
                                                            <div className="w-16 h-24 sm:w-20 sm:h-28 rounded-xl border-2 border-dashed border-[var(--border)] opacity-20" />
                                                        ) : null
                                                    )}
                                                </AnimatePresence>
                                                ...
                                                {playerResult && status === 'FINISHED' && (
                                                    <motion.div
                                                        ...
                                                        className={`absolute -top-2 -right-2 px-2 py-1 rounded-lg border text-[9px] font-black uppercase tracking-wider ${RESULT_CONFIG[playerResult.type]?.bg || 'bg-[var(--bg-hover)] border-white/20'} ${RESULT_CONFIG[playerResult.type]?.color || 'text-white'}`}
                                                    >
                                            ...
                                            {/* Player info card */}
                                            <div className={`
                                                relative px-2.5 py-1.5 rounded-xl border transition-all duration-300 min-w-[90px] sm:min-w-[110px] text-center
                                                ${isActive 
                                                    ? 'border-emerald-400/60 bg-[var(--bg-elevated)]merald-500/10 shadow-[0_0_25px_rgba(16,185,129,0.3)]' 
                                                    : isMe 
                                            ...
                                            `}>
                                                ...
                                                {isActive && (
                                                    <div className="absolute -inset-1 rounded-xl border border-emerald-400/20 animate-ping"/>
                                                )}
                                                    ...
                                                    <div className="flex items-center justify-center gap-0.5 mt-1">
                                                        {[0,1,2].map(i => (
                                                            <motion.div key={i} animate={{ y: [0, -3, 0] }} transition={{ delay: i * 0.12, repeat: Infinity, duration: 0.6 }}
                                                                className="w-1 h-1 rounded-full bg-[var(--bg-elevated)]merald-400"/>
                                                        ))}
                                                    </div>
                                                )}
                                            </div>
                                        ...
                                        <div className="flex flex-col items-center gap-2 mt-8 opacity-50 hover:opacity-90 transition-opacity">
                                            <div className="w-16 h-24 sm:w-20 sm:h-28 rounded-xl border-2 border-dashed border-[var(--border)] bg-white/2"/>
                                            <button
                                                onClick={() => handleAction('JOIN', idx)}
                                                disabled={loadingAction || myPlayerIndex !== -1 || status !== 'WAITING'}
                                                className="flex flex-col items-center gap-1 px-4 py-2 rounded-xl border border-dashed border-white/20 bg-white/3 hover:bg-white/8 hover:border-white/40 transition-all disabled:pointer-events-none min-w-[100px] sm:min-w-[120px]"
                                            >
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {status === 'WAITING' && (
                            <motion.div key="waiting-controls"
                                ...
                                className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-black/40 border border-white/8 rounded-xl px-4 sm:px-6 py-3 sm:py-4 backdrop-blur-md"
                            >
                                    ...
                                    {myPlayerIndex !== -1 && myPlayer?.status === 'IDLE' ? (
                                        <div className="flex items-center gap-2 flex-1">
                                            ...
                                            <div className="flex gap-1">
                                                {[minimum_bet, minimum_bet * 2, minimum_bet * 5].filter(v => v <= userChips).map(v => (
                                                    <button key={v} onClick={() => setBetInput(v)}
                                                        className={`px-2.5 py-1.5 rounded-lg text-[9px] font-black border transition-all ${betInput === v ? 'bg-[var(--bg-secondary)]mber-500/20 border-amber-500/50 text-amber-400' : 'bg-[var(--bg-hover)] border-[var(--border)] text-[var(--text-secondary)] hover:text-white'}`}>
                                                        {v}
                                                    </button>
                                                ))}
                                            </div>
                                            <input
                                                ...
                                                className="w-24 bg-[var(--bg-hover)] border border-[var(--border)] focus:border-amber-500/50 rounded-xl px-3 py-2 text-white font-bold text-sm outline-none transition-colors tabular-nums"
                                                min={minimum_bet}
                                                max={userChips}
                                            />
                                            <button
                                                ...
                                                className="px-5 py-2.5 bg-[var(--bg-secondary)]mber-500 hover:bg-[var(--bg-secondary)]mber-400 disabled:bg-[var(--bg-hover)] disabled:text-[var(--text-tertiary)] rounded-xl text-black font-black text-xs uppercase tracking-widest transition-all shadow-[0_0_20px_rgba(245,158,11,0.3)] disabled:shadow-none"
                                            >
                                        ...
                                        <div className="flex items-center gap-2 text-emerald-400">
                                            ...
                                            <span className="text-xs text-[var(--text-tertiary)]">Waiting for round to start...</span>
                                        </div>
                                    ) : myPlayerIndex === -1 ? (
                                        <span className="text-[var(--text-tertiary)] text-xs font-bold">Sit at a seat above to join the next round</span>
                                    ) : (
                                        <span className="text-[var(--text-tertiary)] text-xs font-bold">Spectating this round...</span>
                                    )}
                                </div>
                        ...
                        {status === 'PLAYING' && (
                            <motion.div key="playing-controls"
                                initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
                                className="bg-black/40 border border-white/8 rounded-xl px-4 sm:px-6 py-3 sm:py-4 backdrop-blur-md"
                            >
                                {isMyTurn ? (
                                    <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[var(--text-tertiary)] text-[10px]">Hand: <span className="text-white font-bold">{myPlayer?.cards ? calculateHandValue(myPlayer.cards, false).total : 0}</span></p>
                                            </div>
                                        </div>
                                        <div className="flex gap-2 flex-wrap justify-center">
                                            ...
                                            <button onClick={() => handleAction('STAND')} disabled={loadingAction}
                                                className="px-4 py-2.5 rounded-xl border border-white/15 bg-[var(--bg-hover)] hover:bg-[var(--bg-hover)] text-white font-black text-xs uppercase tracking-widest transition-all disabled:opacity-40">
                                                Stand
                                            </button>
                                            ...
                                            <button onClick={() => handleAction('HIT')} disabled={loadingAction}
                                                className="px-6 py-2.5 rounded-xl bg-[var(--bg-elevated)]merald-600 hover:bg-[var(--bg-elevated)]merald-500 text-white font-black text-xs uppercase tracking-widest transition-all shadow-[0_0_20px_rgba(16,185,129,0.4)] disabled:opacity-40 disabled:shadow-none flex items-center gap-1.5">
                                        ...
                                        <div className="flex items-center gap-1">
                                            {[0,1,2].map(i => (
                                                <motion.div key={i} animate={{ opacity: [0.3, 1, 0.3] }} transition={{ delay: i*0.2, repeat: Infinity, duration: 0.8 }}
                                                    className="w-1.5 h-1.5 rounded-full bg-[var(--bg-elevated)]merald-400"/>
                                            ))}
                                        </div>
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="text-[var(--text-secondary)] text-xs font-bold">Waiting for</p>
                                                <p className="text-white font-black text-sm">{currentTurnPlayer.name}</p>
                                            </div>
                                        </div>
                                        <div className="text-[10px] text-[var(--text-tertiary)] uppercase tracking-widest font-bold">
                                            {currentLiveState.phase === 'PAYOUT' ? 'Resolving payouts...' : 'Waiting...'}
                                        </div>
                                    </div>
                                ) : (
                                    <div className="text-center text-[var(--text-tertiary)] text-xs font-bold uppercase tracking-widest">
                                ...
                                <div className="flex items-center justify-between flex-wrap gap-3">
                                    <div className="flex items-center gap-4 flex-wrap">
                                        ...
                                        {currentLiveState.results?.map((r: any) => (
                                            <div key={r.id} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-[10px] font-black ${RESULT_CONFIG[r.type]?.bg || 'bg-[var(--bg-hover)] border-[var(--border)]'} ${RESULT_CONFIG[r.type]?.color || 'text-white'}`}>
