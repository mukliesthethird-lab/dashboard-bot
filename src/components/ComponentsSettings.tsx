...
const BUTTON_STYLES = [
    ...
    { value: "secondary", label: "Secondary", color: "bg-[var(--bg-hover)]" },
...
export default function ComponentsSettings({ guildId }: { guildId: string }) {
    ...
    return (
        <div className="space-y-8">
            <div className="glass-card rounded-xl p-8 mb-8 overflow-hidden relative">
                ...
                <p className="text-[var(--text-secondary)]">Create reusable interactive components for your messages.</p>
            </div>

            <div className="grid lg:grid-cols-2 gap-8">
                {/* Buttons List */}
                <div className="glass-card rounded-xl border border-[var(--border)] p-6 flex flex-col h-[600px] shadow-none">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-thin scrollbar-thumb-white/20">
                        {components.filter(c => c.type === "button").map(c => (
                            <div key={c.id} className="bg-[var(--bg-hover)] p-4 rounded-xl border border-[var(--border)] hover:border-amber-500/50 transition flex justify-between items-center group">
                                <div>
                                    <div className="font-bold text-white">{c.name}</div>
                                    <div className="font-mono text-xs text-[var(--text-secondary)] bg-[var(--bg-hover)] px-1 rounded inline-block">{c.custom_id}</div>
                                </div>
            ...
            {showEditor && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => setShowEditor(null)}>
                    <div className="bg-[var(--bg-secondary)]/80 backdrop-blur-3xl w-full max-w-5xl max-h-[80vh] rounded-xl border border-white/20 flex flex-col shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between p-6 border-b border-[var(--border)]">
                            <h2 className="text-2xl font-black text-white flex items-center gap-3">
                                {showEditor.type === "button" ? "🔘 Button Editor" : "📋 Select Menu Editor"}
                                <span className="text-sm font-bold text-[var(--text-secondary)] bg-[var(--bg-hover)] px-3 py-1 rounded-full border border-[var(--border)]">{formData.custom_id || 'base_id'}</span>
                            </h2>
                            <button onClick={() => setShowEditor(null)} className="w-10 h-10 rounded-full bg-[var(--bg-hover)] hover:bg-[var(--bg-hover)] text-[var(--text-secondary)] font-bold transition">×</button>
                        </div>

                        <div className="flex-1 overflow-y-auto p-8 space-y-8 bg-transparent">
                            {/* Basic Identity */}
                            <div className="grid grid-cols-2 gap-6 p-6 bg-[var(--bg-secondary)]/80 backdrop-blur-3xl rounded-xl border border-[var(--border)] shadow-none">
                                <div>
                                    <label className="text-xs font-bold text-[var(--text-secondary)] mb-1 block">NAME (DASHBOARD ONLY)</label>
                                    <input type="text" value={formData.name || ''} onChange={e => setFormData(p => ({ ...p, name: e.target.value }))} className="w-full bg-[var(--bg-hover)] border-2 border-[var(--border)] rounded-xl px-4 py-3 text-white focus:outline-none focus:border-amber-400 font-bold" placeholder="e.g. Verify Button" />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-[var(--text-secondary)] mb-1 block">CUSTOM ID (UNIQUE)</label>
                                    <input type="text" value={formData.custom_id || ''} onChange={e => setFormData(p => ({ ...p, custom_id: e.target.value }))} className="w-full bg-[var(--bg-hover)] border-2 border-[var(--border)] rounded-xl px-4 py-3 text-white font-mono focus:outline-none focus:border-amber-400" placeholder="verify_btn" />
                                </div>
                            </div>
                            ...
                            {showEditor.type === "button" && (
                                <div className="space-y-6">
                                    <div className="p-6 bg-[var(--bg-secondary)]/80 backdrop-blur-3xl rounded-xl border border-[var(--border)] shadow-none">
                                        <h3 className="text-lg font-bold text-white mb-4">Button Appearance</h3>
                                        <div className="flex flex-wrap items-end gap-4">
                                            <div>
                                                <label className="text-xs font-bold text-[var(--text-secondary)] mb-1 block">EMOJI</label>
                                                <EmojiPicker value={formData.data?.emoji || ''} onChange={e => updateData({ emoji: e })} guildId={guildId} />
                                            </div>
                                            <div className="flex-1 min-w-[200px]">
                                                <label className="text-xs font-bold text-[var(--text-secondary)] mb-1 block">LABEL</label>
                                                <input type="text" value={formData.data?.label || ''} onChange={e => updateData({ label: e.target.value })} className="w-full bg-[var(--bg-hover)] border-2 border-[var(--border)] rounded-xl px-4 py-3 text-white focus:outline-none focus:border-amber-400" />
                                            </div>
                                            <div>   
                                                <label className="text-xs font-bold text-[var(--text-secondary)] mb-1 block">STYLE</label>
                                    ...
                                    <div className="p-6 bg-[var(--bg-secondary)]/80 backdrop-blur-3xl rounded-xl border border-[var(--border)] shadow-none">
                                        ...
                                        <div className="space-y-4">
                                            <div className="flex items-center gap-4">
                                                <label className="flex-shrink-0 text-sm text-[var(--text-secondary)] font-bold">Cooldown (seconds)</label>
                                                <input type="number" value={formData.data?.cooldown || 0} onChange={e => updateData({ cooldown: parseInt(e.target.value) })} className="w-24 bg-[var(--bg-hover)] border-2 border-[var(--border)] rounded-lg px-3 py-2 text-white focus:outline-none focus:border-amber-400" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="p-6 bg-[var(--bg-secondary)]/80 backdrop-blur-3xl rounded-xl border border-[var(--border)] shadow-none">
                                        ...
                                        <div className="grid grid-cols-3 gap-4 mb-4">
                                            <div className="col-span-3">
                                                <label className="text-xs font-bold text-[var(--text-secondary)] mb-1 block">PLACEHOLDER</label>
                                                <input type="text" value={formData.data?.placeholder || ''} onChange={e => updateData({ placeholder: e.target.value })} className="w-full bg-[var(--bg-hover)] border-2 border-[var(--border)] rounded-xl px-4 py-3 text-white focus:outline-none focus:border-amber-400" />
                                            </div>
                                            <div>
                                                <label className="text-xs font-bold text-[var(--text-secondary)] mb-1 block">MIN VALUES</label>
                                                <input type="number" value={formData.data?.min_values || 1} onChange={e => updateData({ min_values: parseInt(e.target.value) })} className="w-full bg-[var(--bg-hover)] border-2 border-[var(--border)] rounded-xl px-4 py-3 text-white focus:outline-none focus:border-amber-400" />
                                            </div>
                                            <div>
                                                <label className="text-xs font-bold text-[var(--text-secondary)] mb-1 block">MAX VALUES</label>
                                                <input type="number" value={formData.data?.max_values || 1} onChange={e => updateData({ max_values: parseInt(e.target.value) })} className="w-full bg-[var(--bg-hover)] border-2 border-[var(--border)] rounded-xl px-4 py-3 text-white focus:outline-none focus:border-amber-400" />
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-8">
                                            <div className="flex justify-between items-center mb-4">
                                                ...
                                                }} className="text-emerald-500 hover:text-emerald-600 font-bold text-sm bg-emerald-500/10 px-3 py-1.5 rounded-lg transition">+ Add Option</button>
                                            </div>

                                            <div className="space-y-3">
                                                {(formData.data?.options || []).map((opt, idx) => (
                                                    <div key={idx} className="bg-[var(--bg-hover)] border-2 border-[var(--border)] rounded-xl p-4">
                                                        <div className="flex gap-3 mb-3">
                                                            <div className="w-1/3">
                                                                ...
                                                                }} placeholder="Label" className="w-full bg-[var(--bg-secondary)]/80 backdrop-blur-3xl border-2 border-[var(--border)] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-400" />
                                                            </div>
                                                            <div className="w-1/3">
                                                                <input type="text" value={opt.value} onChange={e => {
                                                                    ...
                                                                    updateData({ options: newOpts });
                                                                }} placeholder="Value" className="w-full bg-[var(--bg-secondary)]/80 backdrop-blur-3xl border-2 border-[var(--border)] rounded-lg px-3 py-2 text-white text-sm font-mono focus:outline-none focus:border-amber-400" />
                                                            </div>
...
function ActionManager({ actions, onUpdate, roles, channels, voiceChannels, editingAction, setEditingAction, closeEditor, mini }: any) {
    ...
    return (
        <div className={`rounded-xl border-2 border-[var(--border)] bg-[var(--bg-hover)] ${mini ? 'p-3' : 'p-6'}`}>
            <div className="flex justify-between items-center mb-4">
                <h4 className={`font-bold text-[var(--text-primary)] ${mini ? 'text-sm' : 'text-lg'}`}>⚡ Actions ({actions.length})</h4>
                <div className="relative">
                    <button onClick={() => setShowDropdown(!showDropdown)} className="bg-[var(--bg-hover)] hover:bg-white/20 px-3 py-1 rounded-lg text-[var(--text-primary)] text-xs font-bold transition flex items-center gap-1">
                        + Add Action {showDropdown ? '▲' : '▼'}
                    </button>
                    {showDropdown && (
                        <div className="absolute right-0 top-full mt-2 w-48 bg-[var(--bg-secondary)]/80 backdrop-blur-3xl rounded-xl shadow-xl border border-[var(--border)] z-[60] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                            <div className="fixed inset-0 z-[-1]" onClick={() => setShowDropdown(false)}></div>
                            {ACTION_TYPES.map(t => (
                                <button key={t.value} onClick={() => addAction(t.value)} className="w-full text-left px-4 py-2 text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] text-sm flex items-center gap-2 font-medium transition-colors">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
                {actions.map((act: any, i: number) => (
                    <button key={i} onClick={() => setEditingAction(i)} className="bg-[var(--bg-secondary)]/80 backdrop-blur-3xl hover:bg-[var(--bg-hover)] px-3 py-1.5 rounded-lg text-[var(--text-primary)] text-sm font-bold flex items-center gap-2 border-2 border-[var(--border)] transition">
            ...
            {currentAction && (
                <div className="bg-[var(--bg-secondary)]/80 backdrop-blur-3xl rounded-xl p-4 border-2 border-[var(--border)] mt-4 animate-in fade-in slide-in-from-top-2">
                    ...
                    <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {(currentAction.roles || []).map((rid: string) => {
                                        ...
                                        return r ? <span key={rid} className="px-2 py-1 bg-[var(--bg-hover)] border border-[var(--border)] rounded text-xs text-white flex items-center gap-1 font-bold">
                                            <span className="w-2 h-2 rounded-full" style={{ background: `#${r.color.toString(16).padStart(6, '0')}` }}></span>
                                            {r.name}
                                            <button onClick={() => updateCurrentAction({ roles: currentAction.roles.filter((x: string) => x !== rid) })} className="hover:text-red-500 text-[var(--text-secondary)]">×</button>
                                        </span> : null
                                    })}
                                </div>
                            </div>
                        ...
                        {["send_dm", "send_channel"].includes(currentAction.type) && (
                            <>
                                ...
                                <div>
                                    <label className="text-xs font-bold text-[var(--text-secondary)] mb-1 block">MESSAGE CONTENT</label>
                                    <textarea value={currentAction.message_content || ''} onChange={e => updateCurrentAction({ message_content: e.target.value })} className="w-full bg-[var(--bg-hover)] border-2 border-[var(--border)] rounded-lg px-3 py-2 text-white text-sm min-h-[80px] focus:outline-none focus:border-amber-400" placeholder="Hello there!"></textarea>
                                </div>
                            </>
                        )}
                        ...
                        {currentAction.type === "move_voice" && (
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <input type="number" value={currentAction.user_count || 0} onChange={e => updateCurrentAction({ user_count: parseInt(e.target.value) })} className="w-full bg-[var(--bg-hover)] border-2 border-[var(--border)] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-400" />
                                </div>
                                <div className="flex items-center pt-5">
                                    <label className="flex items-center gap-2 text-sm text-[var(--text-secondary)] cursor-pointer font-bold">
