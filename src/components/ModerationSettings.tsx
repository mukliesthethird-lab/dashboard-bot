    ...
    return (
        <div className="space-y-8 animate-fade-in pb-20 relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Cases Card */}
                <div className="bg-[#2b2d31] rounded-[8px] p-6 flex flex-col justify-between hover:bg-[#313338] transition group">
                    <div>
                        ...
                        <p className="text-[#b5bac1] mb-6">Manage all ban, kick, mute and warn cases.</p>
                    </div>
                    <Link href={`/dashboard/${guildId}/moderation/cases`}>
                        <button className="self-start px-5 py-2.5 bg-[#4e5058] hover:bg-[#686d73] text-white font-medium rounded-[3px] transition flex items-center gap-2">
                            View cases
                        </button>
                    </Link>
                </div>
                ...
                <div className="bg-[#2b2d31] rounded-[8px] p-6 flex flex-col justify-between hover:bg-[#313338] transition group">
                    <div>
                        ...
                        <p className="text-[#b5bac1] mb-6">Manage incoming reports from your community.</p>
                    </div>
                ...
                <div className="flex flex-col gap-4">
                    {menuItems.map((item, index) => (
                        <div
                            ...
                            className={`bg-[#2b2d31] rounded-[8px] p-5 hover:bg-[#35373c] transition-all flex items-center gap-5 group ${!item.isToggle && item.onClick ? 'cursor-pointer' : ''}`}
                        >
                            {/* Icon */}
                            <div className="w-12 h-12 rounded-lg bg-[#313338] text-[#b5bac1] group-hover:text-white flex items-center justify-center text-2xl transition-colors">
                                {item.icon}
                            </div>
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-[#b5bac1] font-medium text-sm">{item.desc}</p>
                            </div>
                            ...
                            <div>
                                {item.isToggle ? (
                                    ...
                                    <div className="w-10 h-10 rounded-[3px] bg-[#313338] text-[#b5bac1] group-hover:bg-[#5865F2] group-hover:text-white flex items-center justify-center transition-colors">
            ...
            {showImmuneRolesModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in" onClick={() => setShowImmuneRolesModal(false)}>
                    <div className="bg-[#313338] rounded-[8px] p-6 w-full max-w-md max-h-[80vh] overflow-y-auto shadow-2xl relative" onClick={e => e.stopPropagation()}>
                        <button onClick={() => setShowImmuneRolesModal(false)} className="absolute top-4 right-4 text-[#b5bac1] hover:text-[#dbdee1]">
                            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                        <h2 className="text-2xl font-bold text-gray-100 mb-2">🛡️ Immune Roles</h2>
                        <p className="text-[#b5bac1] mb-6 text-sm">Select roles that are immune to moderation actions.</p>
                        <div className="space-y-1">
                            {roles.map(role => (
                                <label key={role.id} className="flex items-center gap-3 p-2 rounded-[4px] hover:bg-[#2b2d31] cursor-pointer">
                                    <input
                                        type="checkbox"
                                    ...
                                    />
                                    <span className="font-medium text-[#dbdee1]">{role.name}</span>
                                </label>
                            ))}
                        </div>
                        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-[#1e1f22]">
                            <button
            ...
            {showReasonsModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in" onClick={() => setShowReasonsModal(false)}>
                    <div className="bg-[#313338] rounded-[8px] p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto shadow-2xl relative" onClick={e => e.stopPropagation()}>
                        <button onClick={() => setShowReasonsModal(false)} className="absolute top-4 right-4 text-[#b5bac1] hover:text-[#dbdee1]">
                            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                        <h2 className="text-2xl font-bold text-gray-100 mb-2">📋 Predefined Reasons</h2>
                        <p className="text-[#b5bac1] mb-6 text-sm">Create shortcuts for common punishment reasons.</p>
                        ...
                        <div className="flex gap-2 mb-4">
                            <input
                                ...
                                className="flex-1 px-3 py-2 bg-[#1e1f22] text-[#dbdee1] placeholder-[#87898c] rounded-[3px] focus:outline-none focus:ring-1 focus:ring-[#5865F2]"
                            />
                            <input
                                type="text"
                                ...
                                className="flex-[2] px-3 py-2 bg-[#1e1f22] text-[#dbdee1] placeholder-[#87898c] rounded-[3px] focus:outline-none focus:ring-1 focus:ring-[#5865F2]"
                            />
                            <button
                        ...
                        <div className="space-y-2">
                            {Object.entries(settings.predefined_reasons).map(([key, value]) => (
                                <div key={key} className="flex items-center gap-3 p-3 bg-[#2b2d31] rounded-[4px]">
                                    <span className="px-2 py-1 bg-[#1e1f22] text-[#dbdee1] font-mono text-sm rounded-[3px]">{key}</span>
                                    <span className="flex-1 text-[#b5bac1] truncate text-sm">{value}</span>
                                    <button
            ...
            {showPrivacyModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in" onClick={() => setShowPrivacyModal(false)}>
                    <div className="bg-[#313338] rounded-[8px] p-6 w-full max-w-md shadow-2xl relative" onClick={e => e.stopPropagation()}>
                        <button onClick={() => setShowPrivacyModal(false)} className="absolute top-4 right-4 text-[#b5bac1] hover:text-[#dbdee1]">
                            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                        <h2 className="text-2xl font-bold text-gray-100 mb-2">👁️ Privacy Settings</h2>
                        <p className="text-[#b5bac1] mb-6 text-sm">Choose what information to show users about their cases.</p>

                        <div className="space-y-2">
                            <label className="flex items-center justify-between p-3 bg-[#2b2d31] rounded-[4px] cursor-pointer">
                                <span className="font-medium text-[#dbdee1] text-sm">Show moderator name</span>
                                <div
                            ...
                            <label className="flex items-center justify-between p-3 bg-[#2b2d31] rounded-[4px] cursor-pointer">
                                <span className="font-medium text-[#dbdee1] text-sm">Show punishment reason</span>
                                <div
                            ...
                            <label className="flex items-center justify-between p-3 bg-[#2b2d31] rounded-[4px] cursor-pointer">
                                <span className="font-medium text-[#dbdee1] text-sm">Show duration</span>
                                <div
            ...
            {showPunishModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in" onClick={() => setShowPunishModal(false)}>
                    <div className="bg-[#313338] rounded-[8px] p-6 w-full max-w-md shadow-2xl relative" onClick={e => e.stopPropagation()}>
                        <button onClick={() => setShowPunishModal(false)} className="absolute top-4 right-4 text-[#b5bac1] hover:text-[#dbdee1]">
                            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                        <h2 className="text-2xl font-bold text-gray-100 mb-2">🔨 Punish Settings</h2>
                        <p className="text-[#b5bac1] mb-6 text-sm">Set default values for punishments.</p>

                        <div className="space-y-4">
                            <div>
                                <label className="block text-[12px] font-bold text-[#b5bac1] uppercase tracking-wide mb-2">Default Mute Duration</label>
                                <select
                            ...
                            <div>
                                <label className="block text-[12px] font-bold text-[#b5bac1] uppercase tracking-wide mb-2">Default Ban Duration</label>
                                <select
            ...
            {/* Unsaved Changes Bar */}
            {hasChanges && (
                <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-[#111214] border border-[#1e1f22] pl-4 pr-3 py-3 rounded-[8px] shadow-2xl animate-fade-in-up flex items-center gap-6 min-w-[400px]">
