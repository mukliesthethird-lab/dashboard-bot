...
const ShieldToggle = ({ label, desc, active, onToggle }: { label: string; desc: string; active: boolean; onToggle: () => void }) => (
    <div className={`p-3 rounded-xl border transition-all ${active ? 'bg-[#da373c]/5 border-[#da373c]/30 shadow-inner' : 'bg-[var(--bg-tertiary)] border-[var(--border)] hover:border-white/10'}`}>
        <div className="flex items-center justify-between mb-1">
            ...
            <button
                onClick={onToggle}
                className={`relative w-9 h-5 rounded-full transition-colors ${active ? 'bg-[#da373c]' : 'bg-[#4e5058]'}`}
            >
    ...
    return (
        <div className={`space-y-2 ${!active ? 'opacity-30 grayscale cursor-not-allowed' : ''}`}>
            <div className="flex justify-between items-center px-1">
                ...
                <span className="text-[11px] font-black text-[#da373c] bg-[#da373c]/10 px-2 py-0.5 rounded-md border border-[#da373c]/20">{value}{unit}</span>
            </div>
            <div
                ref={trackRef}
            ...
            >
                ...
                {/* Track fill */}
                <div className="absolute left-0 h-1.5 rounded-full bg-[#da373c] pointer-events-none" style={{ width: `${pct}%` }} />
                {/* Thumb */}
                <div
                    className="absolute w-5 h-5 rounded-full bg-[#da373c] border-2 border-white shadow-[0_0_10px_rgba(0,0,0,0.5)] pointer-events-none"
...
export default function AutomodVisualizer({ guildId }: Props) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [automodRes, chanRes] = await Promise.all([
    ...
    const matchedRules = useMemo(() => {
        ...
        for (const word of settings.blacklisted_words) {
            try {
                ...
                if (regex.test(msg)) rules.push(`Blocked: "${word}"`);
            } catch {
                if (msg.includes(word.toLowerCase())) rules.push(`Blocked: "${word}"`);
            }
        }
        return rules;
    ...
    return (
        <div className="space-y-4 max-w-7xl mx-auto animate-fade-up pb-24 px-2">
            ...
            <div className="flex items-center gap-3 mb-1">
                <div>
                    <h1 className="text-lg font-black text-white tracking-tight leading-tight uppercase">Automod Security+</h1>
                    <p className="text-[var(--text-tertiary)] text-[9px] font-bold uppercase tracking-[0.2em]">Active Defense Matrix</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-stretch">
                ...
                <div className="lg:col-span-7 flex flex-col gap-4">
                    <div className="glass-card p-4 rounded-xl border border-[var(--border)] space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-x-8 gap-y-4 pt-3 border-t border-[var(--border)]">
                    ...
                    <div className="glass-card p-4 rounded-xl border border-[var(--border)] flex-1 flex flex-col justify-center">
                        <div className="flex items-center gap-12">
                            <div className="flex-1">
                                <ShieldToggle
                                    label="Emoji Spam" desc="Limits emoji counts."
                                    active={settings.anti_emoji} onToggle={() => updateSetting('anti_emoji', !settings.anti_emoji)}
                                />
                            </div>
                            <div className="flex-[1.5]">
                                <ThresholdSlider
                                    label="Emoji Limit" value={settings.emoji_threshold} min={1} max={50} unit=""
                                    active={settings.anti_emoji} onChange={(v) => updateSetting('emoji_threshold', v)}
                                />
                            </div>
                        </div>
                    </div>
                ...
                {/* Right Column: Controls & Simulator (5/12) */}
                <div className="lg:col-span-5 flex flex-col gap-4">
                    ...
                    {/* Word Filter - More compact here */}
                    <div className="glass-card p-4 rounded-xl border border-[var(--border)] space-y-3">
                        <h3 className="text-[11px] font-black text-white uppercase tracking-widest">
                            Pattern Filter
                        </h3>
                        <form onSubmit={handleAddWord} className="flex gap-2">
                            <input
                                ...
                                className="flex-1 bg-black/20 border border-[var(--border)] rounded-lg px-3 py-1.5 text-white focus:outline-none focus:border-[#da373c] font-mono text-[11px] transition-all"
                            />
                            <button type="submit" className="px-4 bg-[#da373c] hover:bg-red-600 text-white font-black rounded-lg text-sm">+</button>
                        </form>
                        <div className="flex flex-wrap gap-1.5 max-h-[80px] overflow-y-auto p-2 bg-black/40 rounded-lg border border-[var(--border)] content-start custom-scrollbar">
                            ...
                            {settings.blacklisted_words.map((word, idx) => (
                                <div key={`${word}-${idx}`} className="flex items-center gap-1.5 bg-[#da373c]/10 border border-[#da373c]/20 text-red-200 px-2 py-1 rounded-md text-[10px] font-mono group hover:border-[#da373c] transition-all">
                                    <span className="truncate max-w-[80px]">{word}</span>
                                    <button onClick={() => removeWord(idx)} className="text-[#da373c] hover:text-white">✕</button>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    {/* Simulator - Tightened */}
                    <div className={`glass-card p-4 rounded-xl border-2 transition-all flex-1 flex flex-col ${testMessage === '' ? 'border-[var(--border)]' :
                        matchedRules.length > 0 ? 'border-red-500/40 bg-red-500/[0.02]' : 'border-emerald-500/20 bg-emerald-500/[0.02]'
                        }`}>
                        <div className="flex justify-between items-center mb-3">
                            <h3 className="text-[11px] font-black text-white uppercase tracking-widest">Simulator</h3>
                            {testMessage && (
                        ...
                        <textarea
                            ...
                            className="w-full flex-1 bg-black/40 border border-[var(--border)] rounded-lg px-3 py-2 text-white focus:outline-none focus:border-[#da373c] transition-all resize-none font-mono text-[11px] shadow-inner min-h-[80px]"
                        />
                        {testMessage && matchedRules.length > 0 && (
                            <div className="mt-2 flex flex-wrap gap-1">
                                {matchedRules.map((rule, i) => (
                                    <span key={i} className="px-1.5 py-0.5 bg-red-500/10 border border-red-500/20 text-red-200 text-[9px] font-mono rounded">{rule}</span>
                                ))}
                            </div>
                        )}
                    </div>
